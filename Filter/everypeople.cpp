...
public:
	...
	string patstring;
	string personName;
	UpsPattern(double myAR, double myRR, double myTypical, string patname,string person)
		:ar(myAR), rr(myRR), typical(myTypical), patstring(patname), personName(person){
...
int main()
{
	...
	*/
	...
	for (int k = 0; k < names.size(); k++) {
		...
		const string out_path = out_dir + names[k] + ".csv";//.txt  .tsv
		cout << out_path << endl;
		...
		for (auto aUmTypical : umTypical) {
			...
			UpsPattern temp(umAR[alpha], umRR[alpha], umTypical[alpha], alpha, names[k]);
	...
	//输出每个人TopN的模式到一个文件中
	stringstream ss;
	ss <<"personName" << tab<< "AR" << tab << "RR" << tab << "Typical" << tab << "alpha" << endl;
	for (auto aAllTopNVec : allTopNVec) {
		ss << aAllTopNVec.personName << tab << aAllTopNVec.ar << tab << aAllTopNVec.rr << tab << aAllTopNVec.typical << tab << aAllTopNVec.patstring;
