...
//						allTopNVecHigh.push_back(upsPatternVec[j]);
//					}
//				}
//				else {
...
//				if (upsPatternVec.size() > topNumber) {
//					for (int j = 0; j < topNumber; j++) {
//						allTopNVecLow.push_back(upsPatternVec[j]);
//					}
//				}
//				else {
//					for (int j = 0; j < upsPatternVec.size(); j++) {
...
//
//	//这里是highLow中的代码
//	unordered_map<string, double> umTypicalHigh;//高分组典型度
//	unordered_map<string, double> umTypicalLow;//低分组典型度
//
//	for (int k = 0; k < categoryNames.size(); k++) {
//		string category = categoryNames[k];//low  high categoryNames[k]
//		if (category == "high") {
...
//		string category = categoryNames[k];//low  high categoryNames[k]
//		stringstream ss;
//		ss << "personName" << tab << "AR" << tab << "RR" << tab << "Typical" << tab << "questionid" <<
//			tab << "高分组支持度" << tab << "低分组支持度" << tab << "高分组-低分组支持度" << tab <<
//			"低分组-高分组支持度" << tab << "高分组典型度" << tab << "低分组典型度" << tab << "alpha" << endl;
//		if (category == "high") {
//			allTopNVec = allTopNVecHigh;
//		}
//		else {
//			allTopNVec = allTopNVecLow;
//		}
//		for (auto aAllTopNVec : allTopNVec) {
//			ss << aAllTopNVec.personName << tab << aAllTopNVec.ar << tab << aAllTopNVec.rr << tab << aAllTopNVec.typical << tab << aAllTopNVec.questionName <<
//				tab << umScsuppAllHigh[aAllTopNVec.patstring] << tab << umScsuppAllLow[aAllTopNVec.patstring] << tab <<
//				umScsuppAllHigh[aAllTopNVec.patstring] - umScsuppAllLow[aAllTopNVec.patstring] << tab <<
//				umScsuppAllLow[aAllTopNVec.patstring] - umScsuppAllHigh[aAllTopNVec.patstring] << tab <<
//				umTypicalHigh[aAllTopNVec.patstring] << tab << umTypicalLow[aAllTopNVec.patstring] << tab <<
//				aAllTopNVec.patstring;
//		}
//		ofstream out_file(out_dir + questionid + "\\" + category + "allSparse.csv", ofstream::out);
//		out_file << ss.str() << endl;
//		cout << ss.str() << endl;
//		out_file.close();
//
